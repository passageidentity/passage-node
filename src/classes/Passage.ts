...
export default class Passage {
   ...
   * @return {Promise<JWK[]>} JWKs for this app.
   */
  async fetchJWKs(resetCache?: boolean): Promise<JWK[]> {
    // use cached value if found
    if (jwkCache !== undefined && jwkCache.length > 0 && !resetCache) {
      return jwkCache;
    }

    const jwks: JWK[] = await axios
      .get(
  ...
  private async _findJWK(kid: string): Promise<JWK | undefined> {
    if (!jwkCache) return undefined;
    const jwk = jwkCache.find((jwk) => jwk.kid === kid);
