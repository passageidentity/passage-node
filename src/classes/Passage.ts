...
export default class Passage {
   ...
   */
    async fetchJWKs(resetCache?: boolean): Promise<{ [kid: string]: JWK }> {
    // use cached value if found
      ...
      !resetCache
        ...
        const jwks: { [kid: string]: JWK } = await axios
            ...
            .then((res) => {
                // @ts-ignore
                // change this to become a map of the JWK keys
                const jwks = res.data.keys;
                const formattedJWKS: { [kid: string]: JWK } = {};
